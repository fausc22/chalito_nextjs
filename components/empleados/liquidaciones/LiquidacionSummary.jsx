...
const SummaryMetric = ({ label, value, accentClass = 'text-foreground' }) => (
...
export function LiquidacionSummary({
  ...
  return (
    <Card id="panel-calculo" className="border-border shadow-sm">
      <CardHeader className="gap-4 border-b border-border pb-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <CardTitle className="text-lg text-foreground">Liquidación calculada</CardTitle>
            <p className="mt-1 text-lg text-foreground">{liquidacion.empleadoNombre}</p>
            <p className="mt-2 text-md font-medium text-foreground">
              Periodo liquidado: {periodTitle}
            </p>
            {periodRangeLabel ? (
              <p className="mt-1 text-md text-muted-foreground">{periodRangeLabel}</p>
            ) : null}
            {ultimaAsistenciaLabel ? (
      ...
      <CardContent className="space-y-5 pt-5">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-border bg-muted/70 p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Movimientos</p>
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
