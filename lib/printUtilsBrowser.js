...
/** Espacio superior en blanco (~5 cm) para escribir a mano; alineado con print-agent topMarginMm */
const TOP_MARGIN_MM = 50;

const PAGE_CSS = `
  @page { size: 58mm auto; margin: 0; }
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Courier New', monospace; font-size: 11px; color: #000; background: #fff; }
  .ticket { width: 58mm; max-width: 58mm; padding: 3mm 2mm; padding-top: ${TOP_MARGIN_MM}mm; margin: 0 auto; }
...
export function printCustomerPayloadBrowser(payload) {
  ...
  const esFacturaC = f.invoiceType === 'C' || f.tipoCmp === 11 || t.hideTaxBreakdown;
  const tituloComprobante = f.isElectronic
    ? `FACTURA ${escapeHtml(f.invoiceType || 'C')}${f.voucherLabel ? ` ${escapeHtml(f.voucherLabel)}` : ''}`
    : `TICKET #${escapeHtml(o.saleNumber || o.saleId)}`;
  ...
  const taxHtml = !t.hideTaxBreakdown && !esFacturaC && Number(t.tax) > 0
    ? `<div class="row"><span>IVA 21%</span><span>${fmt(t.tax)}</span></div>`
    : '';

  const subtotalHtml = !t.hideTaxBreakdown && !esFacturaC && Number(t.subtotal) > 0
    ? `<div class="row"><span>Subtotal</span><span>${fmt(t.subtotal)}</span></div>`
    : '';

  const caeHtml = f.cae
    ? `<div class="sep">------------------------</div>
       <div class="center">CAE: ${escapeHtml(f.cae)}</div>
       ${f.caeExpiresAt ? `<div class="center">Vto CAE: ${escapeHtml(String(f.caeExpiresAt).slice(0, 10))}</div>` : ''}`
    : '';

  const body = `
    <div class="center bold">${escapeHtml((b.name || 'EL CHALITO').toUpperCase())}</div>
    ${b.address ? `<div class="center">${escapeHtml(b.address)}</div>` : ''}
    ${b.cuit ? `<div class="center">CUIT: ${escapeHtml(b.cuit)}</div>` : ''}
    ${b.phone ? `<div class="center">Tel: ${escapeHtml(b.phone)}</div>` : ''}
    <div class="sep">------------------------</div>
    <div class="center bold">${tituloComprobante}</div>
    <div class="center">${escapeHtml(o.createdAtLabel || '')}</div>
    ${o.number || o.id ? `<div class="center">Pedido #${escapeHtml(o.number || o.id)}</div>` : ''}
    <div class="sep">------------------------</div>
  ...
  return openPrintWindow(`Factura #${o.saleNumber || o.saleId}`, body);
}
