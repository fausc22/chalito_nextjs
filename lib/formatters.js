...
export const formatDateTimeSafe = (date, fallback = 'Sin registros') => {
 ...
 *
 * @param {Object} params - { calle, numero, entreCalles, edificio, piso }
 * @returns {string} Ej: "Calle: Av. Belgrano | Altura: 1234 | Entre calles: 20 y 22 | Edificio/Casa: Torre A | Piso/Depto: 3° A"
 */
export const formatDireccionEntrega = ({ calle = '', numero = '', entreCalles = '', edificio = '', piso = '' } = {}) => {
  const trim = (v) => (typeof v === 'string' ? v.trim() : '') || '';
  const c = trim(calle);
  const n = trim(numero);
  const ec = trim(entreCalles);
  ...
  if (ec) partes.push(`Entre calles: ${ec}`);
 ...
 * Devuelve { calle, altura, edificioCasa, pisoDepto, entreCalles }.
 * Soporta formato nuevo (detallado) y antiguo (comas).
 *
 * @param {string} str - cliente_direccion del pedido
 * @returns {{ calle: string, altura: string, edificioCasa: string, pisoDepto: string, entreCalles: string }}
 */
export const parseClienteDireccion = (str) => {
  const empty = { calle: '', altura: '', edificioCasa: '', pisoDepto: '', entreCalles: '' };
  ...
  if (/Calle:\s*/i.test(s) || /Altura:\s*/i.test(s)) {
    ...
    for (const p of parts) {
      ...
      else if (/^Entre calles:\s*/i.test(p)) result.entreCalles = p.replace(/^Entre calles:\s*/i, '').trim();
  ...
  return { calle, altura, edificioCasa, pisoDepto, entreCalles: '' };
};

/**
 * Mapea cliente_direccion / ultima_direccion al shape del formulario de pedidos.
 * @param {string} str
 * @returns {{ calle: string, numero: string, edificio: string, piso: string, entreCalles: string }}
 */
export const direccionFormFromStored = (str) => {
