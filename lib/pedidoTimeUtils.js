...
/**
 * Minutos hasta la ventana automática de preparación (horario_entrega - tiempo_estimado).
 * Valor negativo = la ventana ya pasó o estamos dentro de ella.
 */
export const getMinutosHastaVentanaPreparacion = (pedido) => {
  const horaProgramada = pedido?.horaProgramada || pedido?.horario_entrega || pedido?.horarioEntrega;
  ...
  try {
    ...
    if (typeof horaProgramada === 'string' && horaProgramada.includes('-')) {
      const fechaStr = horaProgramada.includes('T') ? horaProgramada : horaProgramada.replace(' ', 'T');
      horaObjetivo = new Date(fechaStr);
    } else if (/^\d{1,2}:\d{2}/.test(String(horaProgramada))) {
      const partes = String(horaProgramada).split(':');
      horaObjetivo = new Date();
      horaObjetivo.setHours(parseInt(partes[0], 10), parseInt(partes[1], 10), 0, 0);
    } else {
      horaObjetivo = new Date(horaProgramada);
    }

    if (Number.isNaN(horaObjetivo.getTime())) return null;

    const tiempoPrep = pedido.tiempoEstimadoPreparacion ?? 15;
    const ventanaMs = horaObjetivo.getTime() - tiempoPrep * 60 * 1000 - Date.now();
    return Math.floor(ventanaMs / 60000);
  } catch {
    return null;
  }
};
