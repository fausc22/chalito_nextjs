...
/** Detección de adicionales de presentación (Hacela doble / "Doble", etc.) */
...
export function separarPresentacionYExtras(extras) {
  const arr = Array.isArray(extras) ? extras : [];
  const presentacion = arr.filter((e) => esExtraDePresentacion(extraNombre(e)));
  const resto = arr.filter((e) => !esExtraDePresentacion(extraNombre(e)));
...
export function getExtrasSinPresentacion(extras) {
  const arr = Array.isArray(extras) ? extras : [];
  return arr.filter((e) => !esExtraDePresentacion(extraNombre(e)));
}
