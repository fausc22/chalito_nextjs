...
const getExtraCantidad = (extra) => {
  const n = parseInt(extra?.cantidad, 10);
  if (!Number.isFinite(n) || n < 1) return 1;
  return n;
};

const sumExtrasUnitario = (extras = []) =>
  extras.reduce(
    (sum, extra) => sum + toNumber(extra?.precio, 0) * getExtraCantidad(extra),
    0
  );
...
export const calculateCartItemTotal = (item) => {
  const snapshotSubtotal = toNumber(item?.subtotalSnapshot, Number.NaN);
  if (Number.isFinite(snapshotSubtotal)) return snapshotSubtotal;

  const quantity = getQuantity(item);
  const unitBasePrice = toNumber(item?.price ?? item?.precio, 0);
  const extras = item?.extras ?? item?.extrasSeleccionados ?? [];

  const snapshotExtrasTotal = toNumber(item?.extrasTotalSnapshot, Number.NaN);
  const unitExtrasTotal = Number.isFinite(snapshotExtrasTotal)
    ? snapshotExtrasTotal
    : sumExtrasUnitario(extras);
